...
class Semigroup a where
    (<>) :: a -> a -> a
    stimes :: Integral b => b -> a -> a {-O s na frente de times, significa que eh um semigrupo-}
    stimes = fastTimes
    sconcat :: NonEmpty a -> a
    sconcat = fastCat {-Versao mais rapida-}
    --sconcat (x :| []) = x
...
instance Functor NonEmpty where
  fmap f (a :| xs) = f a :| fmap {- Esse fmap n eh o fmap, eh o fmap -} f xs

--Implementacao em arvore
instance Functor BTPar where
  fmap f (Leaf x) = Leaf $ f x
  fmap f (Node l r) = Node (fmap f l) (fmap f r)

{- Exercicio 6: Na definicao de Semigroup -}
...
fastFib n = fb
  where
    (Fib [[_, _], [_, fb]]) = stimes (n + 1) (Fib [[1, 1], [1, 0]])
