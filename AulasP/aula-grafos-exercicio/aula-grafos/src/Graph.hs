...
data Vertex = Vertex {
  idf             :: !Int, -- ID da palavra --> o ! obriga o codigo a resolver tudo e processar na hora, deixa de ser preguicoso
  word            :: !Text,
  neighborIndices :: Set Int -- Cjto de IDs das palavras vizinhas (Usa set qnd quero uma lista sem os repetidos)
}
...
neighbors = S.map (g ! v) (neighborIndices v)

-- Devolve o número de vértices em um grafo
size :: Graph -> Int
size = V.length $ toVector g
...
wordIntBinSearch g w = binSearch 0 (size g)
  where
    ...
    binSearch l r 
      | l == r = -1 -- nao existe posicao -1
      | w == wMid = mid -- w eh o que estou procurando
      | w < wMid = binSearch l mid
      | otherwise = binSearch (mid + 1) r
      where
        mid = (l + r) `div` 2
        wMid = word (g ! mid) -- meio do vetor
